...
//      Den første person med alderen 44
        System.out.println(findFirst(persons,
                p -> p.getAge() == 44));
//      Den første person med et navn der starter med "S"
        System.out.println(findFirst(persons,
                p -> p.getName().startsWith("S")));
//      Den første person hvis navn indeholder mere end et "i"
        System.out.println(findFirst(persons,
                p -> p.getName().chars().filter(ch -> ch == 'i').count() > 1));
//      Den første person med en alder lig længden af navnet
        System.out.println(findFirst(persons,
                p -> p.getAge() == p.getName().length()));
//      Alle personer med et navn der indeholder "i"
        System.out.println(findAll(persons,
                p -> p.getName().contains("i")));
//      Alle personer der starter med "S"
        System.out.println(findAll(persons,
                p -> p.getName().startsWith("S")));
